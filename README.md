...
<style>
    ...
    @keyframes piscarAnim { 0% { background-color: #ffb3b3; } 50% { background-color: #fff; } 100% { background-color: #ffb3b3; } }
...
<div class="container">
    ...
    <div class="dashboard">
        <p>Total Entradas: <span id="totalEntradas">R$ 0,00</span></p>
        <p>Total Saídas: <span id="totalSaidas">R$ 0,00</span></p>
        <p>Saldo do Caixa: <span id="saldoCaixa">R$ 0,00</span></p>
    </div>
...
function registrarEntrada(valor, motivo){
    let registro = { tipo:"Entrada", valor:Number(valor||0), motivo:motivo||"", data:new Date().toISOString() };
    caixa.push(registro);
...
function atualizarCaixa(){
    ...
    document.getElementById("saldoCaixa").innerText = formatarMoeda(totalEntradas-totalSaidas);
}
...
function marcarPago(i){
    ...
    atualizarTabela();
    calcularTotais();
    registrarEntrada(clientes[i].valorFinal, `Recebimento do cliente ${clientes[i].nome}`);
}
